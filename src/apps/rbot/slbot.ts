...
export const slCallback = async (req: Request, res: Response, next: NextFunction) => {
  ...
  switch (req.path) {
    ...
    case '/sl/get':
      if(_checkToken()){
        res.send([{get: req.query, post: req.body}, await actionSlGetRe(tid, req.body.args)]);
      }
      break;
    case '/sl/transfer':
      if(_checkToken()){
...
async function actionSlGetRe(tid: number, args: string){
  try{
    const res = await getRedEnvelope(tid, [args], getI18n())
    return res
  }catch(e){
    console.log('error:', e)
    return (e as Error).message
